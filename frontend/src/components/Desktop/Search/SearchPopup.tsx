...
const SearchPopup: React.FC<PopupProps> = ({ changeDialogSetting }) => {
    ...
    console.log(teamSearchResults, playerSearchResults)

    return (
        <DialogContent className="min-h-[400px] h-fit">
                ...
                <div>
                    ...
                    <div className="flex w-fit justify-between gap-x-4 mt-2">
                        {teamSearchResults.map((team: TeamSearchResult) => (
                            ...
                            <Link to={team.url} className="max-w-[80px] border border-slate-200 p-4 flex flex-col gap-y-2 items-center justify-center hover:cursor-pointer hover:scale-105">
                ...
                <div className="max-h-[400px]">
                    ...
                    <div className="flex justify-between gap-y-2 flex-wrap mt-2">
                        {playerSearchResults.map((player: PlayerSearchResult) => (
                            <Link to={`${process.env.REACT_APP_FRONTEND_URL}/nba/players/id/${player.player_id}/${player.name.toLowerCase().replace(" ", "-")}`} className="flex p-2 gap-x-2 w-[48%] items-center border border-slate-200 rounded-md hover:cursor-pointer hover:scale-105">
                                <img src={player.photo_url} className="size-12 object-contain" alt={player.name} />

                                <div>
                                    <p className="font-semibold text-xs">{player.name} <span className="font-light">#{player.jersey_number}</span></p>
                                    <p className="text-xs">{player.team_id >= 0 ? shortenTeamName(player.team_id) : "Retired/Waived"} · {convertPlayerPosition(player.player_position)}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
