...
const SwitchPlayerItem = (props: {player: Player}) => {
    ...
    return (
        <Link to={`/nba/players/id/${props.player.player_id}/${props.player.name.replace(" ", "-").toLowerCase()}?view=overview`} className="flex items-center gap-x-4 w-full py-1 hover:underline" key={props.player.player_id}>
