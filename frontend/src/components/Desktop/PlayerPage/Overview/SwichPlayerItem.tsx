...
const SwitchPlayerItem = (props: {player: Player}) => {
    ...
    return (
        <Link to={`/nba/players/id/${props.player.player_id}/${props.player.name.replace(" ", "-").toLowerCase()}?view=overview`} className="flex items-center gap-x-4 w-full py-1" key={props.player.player_id}>
            ...
            <div>
                ...
                <div className="flex gap-x-1 text-sm font-light">
                    ...
                    <p>{props.player.player_position}</p>
                </div>
            </div>
        </Link>
    )
