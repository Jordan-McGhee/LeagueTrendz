...
export const getUser = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(200).json({ message: "Found user!", user: userQueryResult.rows[0]})
}
...
export const signUp = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (nameResponse.rows.length > 0) {
        return res.status(409).json({ message: "This username is taken!", user_id: nameResponse.rows[0].user_id })
    } else if (emailResponse.rows.length > 0) {
        return res.status(409).json({ message: "This email is taken!", user_id: emailResponse.rows[0].user_id })
    }
    ...
    return res.status(201).json({ message: "Successfully created user!", user_id: createUserResponse.rows[0].user_id, email: createUserResponse.rows[0].email, token: token })
}
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(200).json({ message: `login successful!`, user_id: userQueryResult.rows[0].user_id, email: userQueryResult.rows[0].email, token: token })
}
...
export const changePassword = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(201).json({ message: "Updated password!", user:updatePasswordResult.rows[0], user_id: userQueryResult.rows[0].user_id })
}
...
export const changeAdminStatus = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(201).json({ message: `Updated user #${user_id}'s admin status to ${adminStatus}`, user_id: user_id, adminQueryResult: adminQueryResult.rows[0] })
}